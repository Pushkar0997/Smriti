...
- [ ] **M0-SETUP-02** Create Supabase project, enable the `pgvector` extension, create `documents`/`chunks`/`query_counts` tables per `architecture.md` §3 (schema.sql & lib/supabase.ts ready; awaiting manual project creation and SQL execution in dashboard)
